...
import authorsFakerFactory from "./authorsFakerFactory";
...
const AuthorsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = authorsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("authors").create(elem));
...
export default connect(null, mapDispatch)(AuthorsFakerDialogComponent);

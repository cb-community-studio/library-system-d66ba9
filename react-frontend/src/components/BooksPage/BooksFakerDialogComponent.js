...
import booksFakerFactory from "./booksFakerFactory";
...
const BooksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = booksFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("books").create(elem));
...
export default connect(null, mapDispatch)(BooksFakerDialogComponent);

...
import publishersFakerFactory from "./publishersFakerFactory";
...
const PublishersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = publishersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("publishers").create(elem));
...
export default connect(null, mapDispatch)(PublishersFakerDialogComponent);
